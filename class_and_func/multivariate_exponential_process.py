...
class multivariate_powerlaw_hawkes(object):
    """
    Multivariate Hawkes process with exponential kernel. No events nor initial conditions considered.
    """

    def __init__(self, mu, alpha, beta, gamma, max_jumps=None, max_time=None):
        """
        ...
        gamma_radius = np.copy(gamma)
        #gamma_radius[np.sum(alpha, axis=1, keepdims=True) == 0] = 1
        gamma_radius[alpha== 0] = 1
        spectral_radius = np.max(np.abs(np.linalg.eig(np.maximum(alpha, 0) / gamma_radius)[0]))
    ...
    def simulate_jumps(self):
        ...
        upper_intensity = np.copy(self.mu)

        while flag < self.max_jumps:

            upper_intensity = np.sum(upper_intensity)

            t += np.random.exponential(1 / upper_intensity)
            ij_intensity = np.sum([((self.alpha[:, mark-1] * self.beta[:, mark-1]) / ((1 + self.beta[:, mark-1] * (t - time))**(1 + self.gamma[:, mark-1])) ).reshape((self.nb_processes, 1)) for time, mark in self.timestamps[1:]], axis=0)

            candidate_intensities = self.mu + ij_intensity
            ...
            if type_event < self.nb_processes:
                self.timestamps += [(t, type_event + 1)]
                ij_intensity += self.alpha[:, type_event].reshape((self.nb_processes, 1))
                upper_intensity = self.mu + np.sum([((np.maximum(self.alpha[:, mark-1], 0) * self.beta[:, mark-1]) / ((1 + self.beta[:, mark-1] * (t - time))**(1 + self.gamma[:, mark-1])) ).reshape((self.nb_processes, 1)) for time, mark in self.timestamps[1:]], axis=0)
    ...
    def plot_intensity(self, ax=None, plot_N=True):
        ...
        if not self.simulated:
            ...
            for i in range(1, len(self.timestamps[1:])):
                # On définit la fonction à tracer entre T_n et T_{n+1}
                func = lambda t: self.mu + np.sum([((self.alpha[:, mark-1] * self.beta[:, mark-1]) / ((1 + self.beta[:, mark-1] * (t - time))**(1 + self.gamma[:, mark-1])) ).reshape((self.nb_processes, 1)) for time, mark in self.timestamps[1:i+1]], axis=0)

                # On enregistre la division de temps et les sauts
                interval_t = np.linspace(self.timestamps[i][0], self.timestamps[i + 1][0], step)
                times += interval_t.tolist()
                ###print(intensities.shape, np.array(list(map(func, interval_t))).squeeze().T.shape)
                intensities = np.concatenate((intensities, np.array(list(map(func, interval_t))).squeeze().T), axis=1)
                if plot_N:
                    jumps_plot[self.timestamps[i][1]-1] += [self.timestamps[i][0] for t in range(2)]
