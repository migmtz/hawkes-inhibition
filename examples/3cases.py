...
if __name__ == "__main__":
    ...
    ax[1].plot([times2[0], times2[0]], [0, mu - 0.2], c="#1f77b4", linestyle="--")
    ax[1].plot([times2[0], times2[0]], [mu, 0], c="r")
    ax[1].plot(lines2, fun2(lines2), c="#1f77b4", linestyle="--")
    ...
    lines2 = np.linspace(times2[0], times2[1], 100)
    ax[2].plot([times2[0], times2[0]], [0, mu - 0.3], c="#1f77b4", linestyle="--")
    ax[2].plot([times2[0], times2[0]], [mu, 0], c="r")
    ax[2].plot(lines2, fun2(lines2), c="#1f77b4", linestyle="--")
    ax[2].plot(lines2, np.maximum(fun2(lines2), 0), c="r")
    last2 = fun2(lines2[-1])
    lines2last = np.linspace(times2[1], times2[2], 1000)
    fun2last = lambda x: mu - 0.3 * np.exp(-beta * (x - times2[0])) + 0.2 * np.exp(-beta * (x - times2[1]))
    ax[2].plot([times2[1], times2[1]], [last2, 0], c="#1f77b4", linestyle="--")
    ax[2].plot([times2[1], times2[1]], [0, last2 + 0.2], c="r")
    ax[2].plot(lines2last, fun2last(lines2last), c="#1f77b4", linestyle="--")
