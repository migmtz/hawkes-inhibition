...
if __name__ == "__main__":
    ...
    for number in range(1,11):
        ...
        if 14 in orig_dict_filtre.keys():
            print(orig_dict_filtre[14])

            print(number, np.sum([m==orig_dict_filtre[14] for i,m in tList]))
            print([(t,m) for t,m in tList if m == orig_dict_filtre[14]])
    ...
    # fig, ax = plt.subplots()
    # sns.heatmap(list_neurons, ax=ax)
    #
    # fig2, ax2 = plt.subplots()
    # ax2.plot(np.sum(list_neurons, axis=1))
    # plt.show()