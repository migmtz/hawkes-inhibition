...
if __name__ == "__main__":
    ...
    gm = sns.heatmap(mu, ax=ax[0], cmap="Greens", yticklabels=20, cbar=True,
    ...
    g = sns.heatmap(beta, ax=ax[2], cmap="Greens", yticklabels=20, cbar=True,
                    cbar_kws={"orientation": "horizontal", "pad": 0.05})
    # g.set_xticklabels([])
    ...
    if not (only_heatmap):
        ...
        blah = LinearSegmentedColormap.from_list('Custom', hex_list, len(hex_list))
        galt = sns.heatmap(np.sign(heatmap2), ax=axalt, cmap=blah, center=0, annot=False, linewidths=0.0, mask=mask,
                           xticklabels=10,
        ...
        pos_giv = np.sum(heatmap * (heatmap > 0), axis=0) - (
                    heatmap.diagonal() * (heatmap.diagonal() > 0))  # horizontal
        pos_rec = np.sum(heatmap * (heatmap > 0), axis=1) - (heatmap.diagonal() * (heatmap.diagonal() > 0))  # vertical

        neg_giv = np.sum(heatmap * (heatmap < 0), axis=0) - (
                    heatmap.diagonal() * (heatmap.diagonal() < 0))  # horizontal
    ...
    fig.savefig('/Users/alisdair/PycharmProjects/hawkes-inhibition/simulated_data/revision_jcgs/' + labels[0] + '_heatmap.eps',
                format="eps", dpi=300)
    plt.show()