...
class multivariate_estimator_bfgs_conf_thresh(object):
    """
    ...
    def fit(self, timestamps, initial, support, threshold):
        """
        Parameters
        ----------
        ...
        alpha_estim = np.abs(self.res.x[self.dim:-self.dim])
        ordered_alpha = np.sort(alpha_estim)
        #print(ordered_alpha)
...
if __name__ == "__main__":
    ...
    for number in range(1, 11):
        ...
        for i in range(1, dim + 1):
            ...
            if number == 1:
                min_alpha[filtre_dict_orig[i] - 1, :] = np.array(aux)
                max_alpha[filtre_dict_orig[i] - 1, :] = np.array(aux)
            else:
                min_alpha[filtre_dict_orig[i] - 1, :] = np.minimum(aux, min_alpha[filtre_dict_orig[i] - 1, :])
                max_alpha[filtre_dict_orig[i] - 1, :] = np.maximum(aux, max_alpha[filtre_dict_orig[i] - 1, :])
    ...
    for number in range(1, 11):
        ...
        with open("estimation/_traitements2_" + str(number) + 'minmaxgradthresh' + str(threshold*100), 'w', newline='') as myfile:
            wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)

            loglikelihood_estimation = multivariate_estimator_bfgs_conf_thresh(dimension=dim, options={"disp": False})

            print("Starting estimation...")
            start_time = time.time()
            res = loglikelihood_estimation.fit(tList, initial=initial, support=real_support, threshold=threshold)
            end_time = time.time()
