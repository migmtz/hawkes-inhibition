...
def obtain_confidence_intervals(file_name, number, dim, number_estimations):
    ...
    with open("estimation/_traitement2_" + str(number) + file_name, 'r') as read_obj:
        csv_reader = csv.reader(read_obj)
        for row in csv_reader:
            if n < number_estimations:
...
if __name__ == "__main__":
    ...
    for number in range(1, 11):
        ...
        estimation = obtain_average_estimation("minmaxgrad", number, dim, 1)
    ...
    print(np.sum(support)/(alpha.shape[0]**2))
