...
if __name__ == "__main__":
    ...
    fig3ndiag, ax3ndiag = plt.subplots(2, 1)
    heatmap = np.sign(heatmap)
    ...
    ax3ndiag[0].plot(pos_rec[arg_aux], label="Positive receiving", c="g")
    ax3ndiag[0].plot(neg_rec[arg_aux], label="Negative receiving", c="r")
    ax3ndiag[1].plot(pos_giv[arg_aux], label="Positive giving", c="g")
    ax3ndiag[1].plot(neg_giv[arg_aux], label="Negative giving", c="r")
    ax3ndiag[0].legend()
    ax3ndiag[1].legend()
    ...
    fig7, ax7 = plt.subplots(2, 2, gridspec_kw={'width_ratios': [7, 1], "height_ratios": [1, 7]}, figsize=(8,8))
    sns.heatmap(heatmap[arg_aux][:, arg_aux], ax=ax7[1, 0], cmap=blah, center=0, annot=False, linewidths=0.0,
                mask=mask[arg_aux][:, arg_aux], xticklabels=10, yticklabels=10, cbar=False)
