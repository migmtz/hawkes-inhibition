    ...
    for number in [8]:
        ...
        for t,m in tList:
            if m < 9:
                ...
                new_tList += [(t, m-1)]
        #new_tList = [(t, m) for t,m in tList if m != orig_dict_filtre[14]]

        dim = len(filtre_dict_orig)-1 ## El filtre de 14 es 9

        a_file.close()


        with open("estimation/_traitements2_"+str(number) + 'trainweird', 'w', newline='') as myfile:
            ...
            res = loglikelihood_estimation.fit(new_tList)
            end_time = time.time()
