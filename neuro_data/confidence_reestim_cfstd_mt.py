...
def obtain_confidence_intervals(file_name, number, dim, number_estimations):
    ...
    with open("resamples/_resamples_" + str(number) + file_name, 'r') as read_obj:
        csv_reader = csv.reader(read_obj)
        for row in csv_reader:
            if n < number_estimations:
...
if __name__ == "__main__":
    ...
    alpha_dev = alpha_dev / (np.maximum(number_estimations, 0))
    ...
    for number in range(1,21):
        a_file = open("resamples/resample" + str(number) + "", "rb")
        tList, filtre_dict_orig, orig_dict_filtre = pickle.load(a_file)
        lists = [int(i) for i in orig_dict_filtre.keys()]
        real_support = support[np.array(lists)-1, :][:, np.array(lists)-1]
        ...
        with open("estimation_resamples/_resamples_" + str(number) + 'stdMT', 'w', newline='') as myfile:
