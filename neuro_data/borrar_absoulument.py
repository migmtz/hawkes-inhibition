...
if __name__ == "__main__":
    ...
    aux = alpha[estimated_mask[0], :][:, estimated_mask[0]]
    aux = np.sign(aux)#*np.exp(aux)
    heatmap = aux/(beta[estimated_mask[0], :])
    ...
    g = sns.heatmap(heatmap, ax=ax, cmap=blah, center=0, annot=False, linewidths=.5, mask=mask, xticklabels=10, yticklabels=10, square=True)
