...
if __name__ == "__main__":
    ...
    with open("estimation_" + str(number) + '_file/_simulation' + str(number), 'r') as read_obj:
        csv_reader = csv.reader(read_obj)
        with open("estimation_" + str(number) + '_file/_estimation' + str(number) + 'confidencegrad', 'w', newline='') as myfile:
            i = 1
            ...
            for row in csv_reader:
                while i <= first:
                    ...
                    res = loglikelihood_estimation.fit(tList, initial=avg, support=support)
    ...
    with open("estimation_" + str(number) + '_file/_simulation' + str(number), 'r') as read_obj:
        csv_reader = csv.reader(read_obj)
        with open("estimation_" + str(number) + '_file/_estimation' + str(number) + 'confidencegrad', 'a', newline='') as myfile:
            i = 1
            wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
            for row in csv_reader:
                if i <= before:
