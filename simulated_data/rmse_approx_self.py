...
def riemann_center_approx(f, a, b, n):
    x = np.linspace(a, b, n)
    return ((b-a) / (n-1)) * np.sum([f((x0+x1)/2) for x0, x1 in zip(x[:-1], x[1:])])


def multivariate_loglikelihood_approx_integral(theta, tList, nb_points, dim=None, dimensional=False):
    """
...
Returns
    ...
    for tc, mc in timestamps[2:]:
        ...
        aux = [riemann_center_approx(lambda x: np.maximum(0, mu[k] + (ic[k] - mu[k]) * np.exp(-beta[k] * (x - tb))), tb, tc, nb_points) for k in range(dim)]
...
class multivariate_estimator_bfgs_integrate_approx(object):
    ...
    def fit(self, timestamps, nb_points):
        """
        Parameters
        ----------
        ...
        self.res = minimize(self.loss, self.initial_guess, method="L-BFGS-B",
                            args=(timestamps, nb_points, self.dim), bounds=self.bounds,
...
if __name__ == "__main__":
    ...
    points_list = range(2, 11)
    points_list = range(11, 16)
    ...
    for nb_points in points_list:

        computation_time = 0

        with open("estimation_" + str(number) + '_file/_simulation' + str(number), 'r') as read_obj:
            csv_reader = csv.reader(read_obj)
            with open("revision_jcgs/estimation_approx_riemann/_estimation_approx_" + str(nb_points), 'a', newline='') as estim_file:
                ...
                for i, row in enumerate(csv_reader):
                    if i < until:
                        ...
                        res = loglikelihood_estimation.fit(tList, nb_points)
                        end_time = time.time()
        ...
        with open("revision_jcgs/computation_times.txt", "a") as write_obj:
            ...
            wr.writerow([str(nb_points) + method + " with precision of " + str(nb_points) + " until " + str(until) + " : " + str(computation_time)])
